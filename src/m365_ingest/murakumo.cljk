...
(defn collection
  [name]
  (str "com.etzhayyim.m365-ingest." name))

(def cell-specs {
  :enumerate-tenant-users {:legacy-cell "enumerate-tenant-users"
     :phase :event
     :murakumo-node "reuben"
     :collections [(collection "enumerate-tenant-users")]
     :required-gates common-gates
     :trigger "manifest cell enumerate-tenant-users"
     :ceiling "Manifest-driven migration scaffold; explicit execution stays in runtime methods"}
  :delta-sync-all-users {:legacy-cell "delta-sync-all-users"
     :phase :event
     :murakumo-node "reuben"
     :collections [(collection "delta-sync-all-users")]
     :required-gates common-gates
     :trigger "manifest cell delta-sync-all-users"
     :ceiling "Manifest-driven migration scaffold; explicit execution stays in runtime methods"}
  :sync-single-user {:legacy-cell "sync-single-user"
     :phase :event
     :murakumo-node "reuben"
     :collections [(collection "sync-single-user")]
     :required-gates common-gates
     :trigger "manifest cell sync-single-user"
     :ceiling "Manifest-driven migration scaffold; explicit execution stays in runtime methods"}
  :syncuser {:legacy-cell "com-etzhayyim-apps-m365Ingest-syncUser"
     :phase :event
     :murakumo-node "reuben"
     :collections [(collection "syncuser")]
     :required-gates common-gates
     :trigger "manifest cell syncuser"
     :ceiling "Manifest-driven migration scaffold; explicit execution stays in runtime methods"}
